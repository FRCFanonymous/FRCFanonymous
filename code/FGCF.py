...
class FGCF(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default(): 
            ...
            self.FGCF = self.permutate(nonzero_embeddings, regularizer)
            self.cFM = self.FGCF
            self.FGCF = tf.reduce_sum(self.FGCF, 1)
            self.FGCF = tf.nn.dropout(self.FGCF, self.dropout_keep[1]) # dropout at the FM layer

            if not self.NFM:
                self.FGCF = tf.reduce_sum(self.FGCF, 1, keepdims=True)  # None * 1
            else:
                self.FGCF = self.deep_FM(self.FGCF, regularizer)

            # _________out _________
            self.feature_analysis = self.FGCF
